...
exports.register = async (req, res) => {
  const { nom, prenom, email, password,role ,forma} = req.body;
  try {
    const newUser = new User({ nom, prenom, email, password,role,forma});
    if (req.files.length > 0) {
      req.files.map(file => {
        newUser.receipt = "http://127.0.0.1:3000/public/" + file.originalname
      })
    } else {
      newUser.receipt = "http://127.0.0.1:3000/public/default.png"
    }
...
exports.GetAllUser = async (req,res)=>{
  try {
    const result = await User.find().populate("forma").populate("publication")
...
exports.GetOneUser = async (req, res) => {
  try {
        const result = await User.findOne({ _id: req.params.id }).populate("forma").populate("publication")
